...
- **`data/input/`**:
    - `Samhita_corrected.txt`: The master unified source file for the Jaimineeya Samhita.
    - `Aaranam_latest.txt`: The master source for the Aaranam collection.
    - `Rik Reconciliation table.xlsx`: The source of truth for Rishi/Devata metadata.
- **`data/output/`**:
    - `Vargeekaran.json`: The processed, hierarchical database of the Samhita.
    - `Aaranam_vargeekaran.json`: The processed database of the Aaranam.
    - `JSV_Rik_Table.csv`: A flattened table used for corrections and analysis.

### **B. The Logic Engine (`src/`)**
- **`generate_json.py`**: The primary parser that converts plain-text Vedic markup into structured JSON.
- **`generate_rik_table.py`**: Integrates metadata from Excel sheets into the main JSON database.
- **`generate_website.py`**: The "Static Site Generator" that creates the beautiful parchment-styled web pages.
- **`render_pdf.py`**: A complex wrapper for LuaLaTeX that produces high-quality Vedic PDFs with accurate accents.
- **`pipeline_config.yaml`**: The central switchboard for mapping input files to output paths.

### **C. The Web Engine (`docs/`)**
- **`docs/index.html`**: The project gateway (Dashboard).
- **`main.js`**: The navigation and client-side search engine.
- **`styles.css`**: The design system, including font-specific Vedic accent alignments.
- **`search-index.js`**: An offline-ready database for ultra-fast mantra searching.
